...
class UserController extends AbstractController {
        ...
        if(!$users){
            ...
            return $render->renderPage(
                ...
                [
                    'title' => 'Список пользователей в хранилище',
                    'users' => $users,
                    'isAdmin' => Auth::isAdmin() // Проверка на админа
                ]
            );
        }
    }
    ...
    public function actionDelete(): string {
        if (!isset($_POST['user_id']) || !is_numeric($_POST['user_id'])) {
            return json_encode(['success' => false, 'message' => 'Неверный идентификатор пользователя']);
        }

        $userId = (int)$_POST['user_id'];
        $result = User::deleteFromStorage($userId);
        if ($result) {
            return json_encode(['success' => true]);
        } else {
            return json_encode(['success' => false, 'message' => 'Не удалось удалить пользователя']);
        }
    }
